...
extension WebServices {
    //rule 1: cannot define stored variables in an extension
    //rule 2: can extend any object, anywhere, anytime
    //rule 3: extensions can be in any file in the project and still work
    
    // Step 5: Create generic function to get single model back
    func getObject<S: NetworkModel>(_ model: S, completion: @escaping (_ object: S?, _ error: String?) -> Void) {
    ...
    func logoutUser<T: NetworkModel>(_ user: T, completion:@escaping (_ user: T?, _ error: String?) -> ()) {
        request(WebServices.shared.baseURL + user.path(), method: user.method(), parameters: user.toDictionary(), encoding: URLEncoding.default)
            .responseJSON { (response) in
                WebServices.parseResponseObject(response: response, completion: completion)
        }
