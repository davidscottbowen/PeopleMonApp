...
class WebServices: NSObject {
    ...
    private override init() { } //constructor that cannot be called outside of this class, so more instances cannot be created
    
    //baseURL private and public vars
    fileprivate var _baseURL = ""
    var baseURL : String { //this provides a wrapper for _baseURL
        get {
            return _baseURL
        }
        set {
    ...
    fileprivate var authTokenExpireDate: String? {
        get {
            let myValet = VALValet(identifier: Constants.keychainIdentifier, accessibility: .whenUnlocked) //valet data can only be accessed when the phone is unlocked
    ...
    func userAuthTokenExpired() -> Bool {
        if self.authTokenExpireDate != nil {
            ...
            if let expireDate = dateFormatter.date(from: dateString) {
                ...
                if expireDate.compare(Date()) == .orderedAscending {
                    return true
                } else {
                    return false
                }
    ...
    //Looks at request type, and given a network model will create the appropriate URL request
